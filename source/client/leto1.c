...
static LETOCONNECTION letoConnPool[ MAX_CONNECTIONS_NUMBER ];
LETOCONNECTION * pCurrentConn = NULL;
static SHORT iConnectRes = 1;
...
static long int leto_Recv( HB_SOCKET_T hSocket )
{
   ...
   do
   {
      ...
      else
      {
         ...
         if( lLen == 0 && ( iLenLen = (((int)*szBuffer) & 0xFF) ) < 10 && (ptr-szBuffer) > iLenLen )
            lLen = leto_b2n( szBuffer+1, iLenLen );
   ...
   // leto_writelog( szBuffer,ptr - szBuffer );
...
static int leto_SendRecv( LETOAREAP pArea, char * sData, ULONG ulLen, int iErr )
{
   ...
   lRet = leto_DataSendRecv( pArea->pConnection, sData, ulLen );
   if( !lRet )
      commonError( pArea, EG_DATATYPE, 1000, 0, NULL, 0 );
   else if( *szBuffer == '-' && iErr )
   {
      commonError( pArea, EG_DATATYPE, iErr, 0, szBuffer, 0 );
      return 0;
   }
   return lRet;
}
...
LETOCONNECTION * leto_ConnectionNew( const char * szAddr, int iPort, const char * szUser, const char * szPass )
{
   HB_SOCKET_T hSocket;
   char szData[256];
   char szBuf[36];
   ...
   iConnectRes = LETO_ERR_MANY_CONN;
   if( i < MAX_CONNECTIONS_NUMBER )
   {
      hSocket = hb_ipConnect( szAddr, htons( iPort ), 5000 );
      if( !hb_iperrorcode() )    
      {
         letoConnPool[i].hSocket = hSocket;
         letoConnPool[i].iPort = iPort;
         letoConnPool[i].pAddr = (char*) hb_xgrab( strlen(szAddr)+1 );
         memcpy( letoConnPool[i].pAddr, szAddr, strlen(szAddr) );
         letoConnPool[i].pAddr[strlen(szAddr)] = '\0';
         if( leto_Recv( hSocket ) )
         {
            char * ptr, * pName, **pArgv;
            char szFile[_POSIX_PATH_MAX + 1];

            // leto_writelog( szBuffer,0 );
            ptr = strchr( szBuffer,';' );
            if( ptr )
            {
               memcpy( letoConnPool[i].szVersion, szBuffer, ptr-szBuffer );
               letoConnPool[i].szVersion[ptr-szBuffer] = '\0';
               ptr ++;
               if( *ptr++ == 'Y' )
                  letoConnPool[i].bCrypt = 1;
               else
                  letoConnPool[i].bCrypt = 0;
               if( *ptr == ';' )
                  letoConnPool[i].cDopcode[0] = '\0';
               else
               {
                  letoConnPool[i].cDopcode[0] = *ptr++;
                  letoConnPool[i].cDopcode[1] = *ptr;
               }
            }
            else
            {
               memcpy( letoConnPool[i].szVersion, szBuffer, strlen(szBuffer) );
               letoConnPool[i].szVersion[strlen(szBuffer)] = '\0';
               letoConnPool[i].cDopcode[0] = '\0';
            }
            ...
            if( letoConnPool[i].cDopcode[0] && szUser )
            {
            ...
            *ptr++ = ';';
            if( letoConnPool[i].cDopcode[0] && szPass )
            {
               ...
               szKey[uiKeyLen] = '\0';
               szKey[0] = letoConnPool[i].cDopcode[0];
               szKey[1] = letoConnPool[i].cDopcode[1];
            ...
            *ptr++ = ';';
            ...
            {
               iConnectRes = LETO_ERR_SEND;
               return NULL;
            }           
            if( !leto_Recv( hSocket ) )
            {
               if( letoConnPool[i].cDopcode[0] )
                  iConnectRes = LETO_ERR_LOGIN;
               else
                  iConnectRes = LETO_ERR_RECV;
               return NULL;
            }
            ptr = leto_firstchar();
            if( !strncmp( ptr, "ACC", 3 ) )
               iConnectRes = LETO_ERR_ACCESS;
            else
            {
               letoConnPool[i].szVerHarbour[0] = '\0';
               if( ( pName = strchr(ptr, ';') ) != NULL && (pName - ptr) >= 3 )
               {
                  memcpy( letoConnPool[i].szAccess, ptr, (pName-ptr>3)? 3 : pName-ptr );
                  if( ( pName = strchr(pName+1, ';') ) != NULL )
                  {
                     memcpy( letoConnPool[i].szVerHarbour, ptr, pName-ptr );
                     letoConnPool[i].szVerHarbour[pName-ptr] = '\0';
                  }
               }
               iConnectRes = 0;
            }
      ...
      else if( !iLenLen && !letoIsBinaryField( pField->uiType, pField->uiLen ) )
      {
         ...
         switch( pField->uiType )
         {
            ...
            case HB_FT_STRING:
               ...
               {
                  commonError( pArea, EG_DATATYPE, 1022, 0, NULL, 0 );
                  return 0;
               }
               if( iLenLen > 0 )
...
static BOOL leto_checkerror( LETOAREAP pArea )
{
   if( *szBuffer == '-' )
   {
      ...
      if( szBuffer[4] == ':' )
      {
         sscanf( szBuffer+5, "%lu-%lu-%lu-%lu", &uiGenCode, &uiSubCode, &uiOsCode, &uiFlags );
         if( ( szFileName = strchr( szBuffer+5, '\t' ) ) != NULL)
            szFileName ++;
      }
      else
      {
...
static ERRCODE letoSkipRaw( LETOAREAP pArea, LONG lToSkip )
{
   ...
   ulDataLen = leto_b2n( szBuffer+1, iLenLen );
   ptr = szBuffer + 2 + iLenLen;
   leto_ParseRec( pArea, ptr );
...
static ERRCODE letoAppend( LETOAREAP pArea, BOOL fUnLockAll )
{
   ...
   if( bFastAppend || leto_PutRec( pArea ) )
      pArea->fRecLocked = 1;
      ...
      case DBRI_LOCKED:
      {
         if( ulRecNo != 0 && ulRecNo != pArea->ulRecNo )
         {
            ...
            else
            {
               ...
               {
                  if( *(szBuffer+3) != '4' )
                     commonError( pArea, EG_DATATYPE, 1021, 0, szBuffer, 0 );
                  return FAILURE;
               }
               ptr = leto_firstchar();
               leto_getCmdItem( &ptr, szData ); ptr ++;
...
static ERRCODE letoClose( LETOAREAP pArea )
{
   ...
   if( bTransActive )
   {
      commonError( pArea, EG_SYNTAX, 1031, 0, NULL, 0 );
      return FAILURE;
   }
   ...
   if( pArea->hTable )
   {
      if( !pArea->pConnection->bCloseAll )
      {
...
static const char * letoRemoveIpFromPath(const char * szPath)
{
   const char * ptr;
...
static LETOCONNECTION * letoOpenConnection( LETOAREAP pArea, LPDBOPENINFO pOpenInfo, char * szFile, BOOL bCreate)
{
   ...
   if( pOpenInfo->ulConnection > 0 && pOpenInfo->ulConnection <= MAX_CONNECTIONS_NUMBER )
   {
   ...
   else
   {
          ...
          !leto_getIpFromPath( (bCreate ? hb_setGetDefault() : hb_setGetPath() ), szAddr, &iPort, szFile, FALSE ) )
      {
         ...
         {
            commonError( pArea, EG_OPEN, (bCreate ? 1 : 101), 0, ( char * ) pOpenInfo->abName, 0 );
            return NULL;
         }
      }
      ...
      {
         commonError( pArea, EG_OPEN, (bCreate ? 1 : 102), 0, ( char * ) pOpenInfo->abName, 0 );
         return NULL;
      }
   }
   return pConnection;
...
static ERRCODE letoInfo( LETOAREAP pArea, USHORT uiIndex, PHB_ITEM pItem )
{
   ...
   switch( uiIndex )
   {
      ...
      {
         hb_itemPutC( pItem, pArea->pConnection->szVersion );
         break;
      }
...
static ERRCODE letoOpen( LETOAREAP pArea, LPDBOPENINFO pOpenInfo )
{
   ...
   {
      if( *(szBuffer+3) == '4' )
         commonError( pArea, EG_OPEN, 103, 32, szBuffer, EF_CANDEFAULT );
      else
         commonError( pArea, EG_DATATYPE, 1021, 0, szBuffer, 0 );
      return FAILURE;
   }
...
static ERRCODE letoInit( LPRDDNODE pRDD )
{
   ...
   hb_ipInit();
   memset( letoConnPool, 0, sizeof(LETOCONNECTION) * MAX_CONNECTIONS_NUMBER );
...
HB_FUNC( LETO_BEGINTRANSACTION )
{
   ...
   if( bTransActive )
   {
      commonError( (LETOAREAP) hb_rddGetCurrentWorkAreaPointer(), EG_SYNTAX, 1031, 0, NULL, 0 );
   }
   else
   {
